...
📢 以下是关于新功能的正在进行中的投票！我建议**所有看到此消息的朋友**参与这个只需 5 秒的匿名投票，以便开发者获得公正的结果。

- 🗳️ [该插件是否应支持兼容 S3 的后端，还是仅保留 WebDAV？](https://github.com/hesprs/obsidian-webdav-sync/discussions/90)
...
- 本插件支持在上传前对你的文件进行加密。
- 加密算法可防止未经授权的文件访问，并能检测意外的修改和移动操作。
- 该加密算法基于更严格的安全威胁模型，在安全性、性能和体积方面均优于类似方案（如 Remotely Save），详见[算法规范](https://github.com/hesprs/obsidian-webdav-sync/blob/main/docs/encryption.md)。
...
- [x] 支持同步 Obsidian 配置文件夹 (`.obsidian/`) 中的文件
- [x] 在 Obsidian Keychain 中保存 WebDAV 凭证
- [x] 允许用户调整速率和并发限制
- [x] 增强同步进度的可观测性
- [x] 实现加密（类似 Remotely Save 的方案）
- [x] 实现自动负载均衡和分块下载，以防止在处理大文件时 Obsidian 崩溃
- [x] 重构同步选择界面
...
虽然本项目与 Nutstore 无关联，但感谢 Nutstore 提供的原型及开源贡献。截至 2026 年 5 月 2 日，约 33% 的工作量由 Nutstore 贡献。
