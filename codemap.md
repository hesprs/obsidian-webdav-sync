...
Obsidian Sync is a WebDAV-first Obsidian plugin that performs two-way synchronization between vault content and remote storage. The repository is organized around scheduler-driven sync execution, deterministic planning/execution, pluggable sync-state persistence, and UI/service layers that make WebDAV sync operable as a general-purpose plugin.
...
- `src/services/sync-scheduler.service.ts` — canonical fan-in for manual/startup/interval/realtime sync requests.
...
| :------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------- |
| `src`                     | Plugin source composition root connecting scheduler/executor services, sync engine, platform adapters, and UI.                                         | [src/codemap.md](./src/codemap.md)                                         |
| `src/components`          | Obsidian UI components for sync controls, confirmations, progress, settings flows, and embedded remote explorer.                                       | [src/components/codemap.md](./src/components/codemap.md)                   |
...
| `src/platform`            | Cross-runtime binary, crypto, and path normalization primitives shared by fs, sync, and utility modules.                                               | [src/platform/codemap.md](./src/platform/codemap.md)                       |
...
| `src/storage`             | Pluggable sync-state persistence boundary (`SyncStateStore`) with default IndexedDB/localspace backend and `SyncRecord` facade for local/remote state. | [src/storage/codemap.md](./src/storage/codemap.md)                         |
...
| `src/components/explorer` | Embedded SolidJS WebDAV explorer used by settings/modals for remote directory selection.                                                               | [src/components/explorer/codemap.md](./src/components/explorer/codemap.md) |
