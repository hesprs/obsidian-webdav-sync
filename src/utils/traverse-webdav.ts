...
import { type DeltaEntry, getDelta } from '~/api/delta';
...
export class ResumableWebDAVTraversal {
	...
	async traverse(): Promise<StatModel[]> {
		return await this.lock.runExclusive(async () => {
			...
			const isIncrementalScan = this.queue.length === 0 && Object.keys(this.nodes).length > 0;
	...
	private async bfsTraverse(): Promise<StatModel[]> {
		...
		while (true) {
			...
			if (traverseStartCursor && traverseStartCursor !== traverseEndCursor) {
				...
				const newCursor = await this.applyDeltaDuringTraversal(traverseStartCursor);
				this.rootCursor = newCursor;

				// If reset occurred, queue is non-empty, need to re-traverse
				if (this.queue.length > 0) {
					logger.info('Reset detected during delta apply, performing full re-scan');
					continue;
				}
	 ...
	 */
	private async applyDeltaDuringTraversal(startCursor: string): Promise<string> {
		let finalCursor = startCursor;
		let processedEntries = 0;

		for await (const { entries, cursor, reset } of this.fetchAllDelta(startCursor)) {
			if (reset) {
				logger.warn(
