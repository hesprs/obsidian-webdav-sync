	...
	settings: {
		...
		encryption: {
			...
			reminderModal: {
				acknowledge: '我知道了',
				messageDisabled:
					'⚠️ 在禁用加密之前，请谨慎考虑以下几点：\n\n1. 所有后续上传将以明文形式进行，不再加密。\n2. 请确保所有设备均已禁用加密。\n3. 如果此仓库此前是加密上传的，请完全删除远程根目录（包括根文件夹），然后重新上传整个仓库。',
				messageEnabled:
					'⚠️ 在启用加密之前，请谨慎考虑以下几点：\n\n1. 所有后续上传都将被加密。\n2. 请确保所有设备都启用了加密并设置了相同的密码。\n3. 如果此仓库此前是未加密上传的，请完全删除远程根目录（包括根文件夹），然后重新上传整个仓库。\n4. 加密算法将解密密钥与文件位置和服务器身份绑定，这提供了更高的安全性和数据完整性。但这也意味着，如果你使用不同的服务器，或者在不使用本插件的情况下将文件移动到其他位置，你将无法解密该文件。\n5. 请避免在服务器上手动管理文件。如果日后更换服务器，请在启用加密的情况下重新上传整个仓库。\n6. 由于加密，同步过程将稍微慢一些。',
				titleDisabled: '已关闭加密',
				titleEnabled: '已启用加密',
			},
		},
