...
export default class CacheClearModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl)
			.addButton((button) => {
				button.setButtonText(i18n.t('settings.cache.clearModal.cancel')).onClick(() => {
					this.close();
				});
			})
			.addButton((button) => {
				...
				button.setButtonText(i18n.t('settings.cache.clear')).onClick(async () => {
					if (confirmed) {
						try {
							...
							button.setButtonText(i18n.t('settings.cache.clearModal.confirm'));
							button.buttonEl.classList.remove('mod-warning');
							confirmed = false;
						}
					} else {
						confirmed = true;
						button.setButtonText(i18n.t('settings.cache.confirm')).setWarning();
					}
				});
